...
import { fetchInspectorFacilityDetail, startActivityWorkflow, updateProgramFacilityProfile } from "../lib/coreApi";
import type { InspectorFacilityDetail } from "../types";
...
function FacilityLocationCard({
  location,
}: {
  location: InspectorFacilityDetail["location"];
}) {
...
function ProfileFields({
  ...
  async function save() {
    ...
    setSaveError(null);
    try {
      const result = await updateProgramFacilityProfile(programFacilityId, draft);
...
export function FacilityDetailPage() {
  ...
  const [data, setData] = useState<InspectorFacilityDetail | null>(null);
  ...
  const loadData = useCallback(() => {
    ...
    fetchInspectorFacilityDetail(facilityId)
      .then((result) => {
        if (cancelled) return;
        setData(result);
        setError(null);
  ...
  async function handleStartWorkflow(activityId: string, workflowId: string) {
    ...
    try {
      await startActivityWorkflow(facilityId, activityId, workflowId);
      loadData();
    } catch {
  ...
  if (isLoading) {
    return (
      <AppLayout title="Facility" breadcrumb={["Inspector", "My Assignments"]}>
  ...
  if (error || !data) {
    return (
      <AppLayout title="Facility" breadcrumb={["Inspector", "My Assignments"]}>
        ...
        <button type="button" className="btn btn-secondary" onClick={() => navigate("/my-assignments")}>
  ...
  return (
    <AppLayout title={data.facility_name ?? "Facility"} breadcrumb={["Inspector", "My Assignments"]}>
      <button type="button" className="btn btn-secondary btn-sm" style={{ marginBottom: "1rem" }} onClick={() => navigate("/my-assignments")}>
        Back to My Assignments
      </button>
      ...
      {data.assignments.map((assignment) => (
        <div className="card" style={{ marginBottom: "1.25rem" }} key={assignment.program_facility_id}>
          ...
          <div className="card-body">
            ...
            <ProfileFields
              ...
              onSaved={(profile) => handleProfileSaved(assignment.program_facility_id, profile)}
            />
                      ...
                      <div style={{ display: "flex", gap: ".5rem", flexWrap: "wrap", marginTop: ".25rem" }}>
                        {activity.workflows.map((workflow) => (
                          <button
                            ...
                            onClick={() =>
                              handleStartWorkflow(activity.program_facility_type_activity_id, workflow.workflow_id)
                            }
                          >
